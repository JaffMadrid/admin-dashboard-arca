...
const formatDetails = (details) => {
	try {
	  if (!details) return '';
	  // If details is already an object, stringify it first
	  ...
	  return Object.entries(detailsObj)
		.map(([key, value]) => {
		  ...
		  // Handle nested objects (like the 'cambios' object)
            ...
            {currentRows.map((bitacora) => (
              ...
              >
                ...
                <td className="px-6 py-4 whitespace-pre-wrap text-sm text-gray-100">
                  {formatDetails(bitacora.detalles)}
                </td>
              </motion.tr>
            ))}
          </tbody>
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        <div className="flex items-center">
          ...
          <span className="text-gray-400 mx-2">
            ...
            {Math.ceil(filteredBitacora.length / rowsPerPage)}
          </span>
          <button
            onClick={() => handlePageChange(currentPage + 1)}
            disabled={
              currentPage === Math.ceil(filteredBitacora.length / rowsPerPage)
            }
            className="text-gray-400 hover:text-gray-300 px-3 py-2"
          >
            <ChevronRight />
