...
const UsersTable = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const res = await fetch(
          "https://admin-dashboard-arca-backend.vercel.app/dashboard/userInfo",
          { method: "GET" }
        );
    ...
    const fetchRoles = async () => {
      try {
        const res = await fetch(
          "https://admin-dashboard-arca-backend.vercel.app/dashboard/roles",
          { method: "GET" }
        );
        const data = await res.json();
  ...
  const handleUserUpdate = async (updatedUser) => {
    try {
      const res = await fetch(
        `https://admin-dashboard-arca-backend.vercel.app/dashboard/updateUser/${updatedUser.id_usuario}`,
        {
          method: "PATCH",
          headers: {
      ...
      if (res.ok) {
        ...
        const updatedRole = roles.find(
          (role) => role.id_rol === data.usuario.id_rol
        );
        ...
        setUsers((prev) =>
          prev.map((user) =>
            user.id_usuario === updatedUserWithRole.id_usuario
              ? updatedUserWithRole
              : user
          )
        );
            ...
            >
              ...
              <div className="mb-3">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setEditUser({ ...editUser, id_rol: e.target.value })
                  }
                >
              ...
              <div className="mb-3">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setEditUser({
                      ...editUser,
                      estado_usuario: e.target.value === "activo",
                    })
                  }
                >
            ...
            <form onSubmit={handleUserCreate}>
              ...
              <div className="mb-3">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setNewUser({ ...newUser, id_rol: e.target.value })
                  }
                  required
                >
                  <option value="" disabled>
	  ...
	  <div className="flex justify-between items-center mt-4">
        ...
        <div className="flex items-center">
          ...
          <span className="text-gray-400 mx-2">
            Página {currentPage} de {Math.ceil(filteredUsers.length / rowsPerPage)}
          </span>
          <button
            onClick={() => handlePageChange(currentPage + 1)}
